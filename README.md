...
### 2026-02-15 — `comment-bot-v2-situational` 태그
- 댓글봇 구조 전환: 규칙 기반 → **상황 기반 프롬프트**
- commonRules 삭제 → 호출별 고유 상황 프레이밍
- call3(troll/misreader) **보호 영역**: 큐레이터 우회, 0~2개 랜덤 직접 삽입
- GPT-5 큐레이터 프롬프트 축소 (8줄 → 2줄)
- 후처리 코드: 마침표/쉼표 제거, 물음표→단정 변환, 쓸데없는 댓글 확률 삽입
- 8단계 파이프라인, GPT 6회 호출 (Azure 5 + OpenAI 1)

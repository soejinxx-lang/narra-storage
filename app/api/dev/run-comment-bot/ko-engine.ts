...
function isEmptyExclamation(s: string): boolean {
    const t = s.trim();
    // 내용어(3글자 이상 한글 단어)가 있으면 통과 — DC식 단문 "이번화 ㄷㄷ", "이거 수상함" 살림
    const hasContentWord = /[가-힣]{3,}/.test(t);
    if (hasContentWord) return false;
    // 내용어 없이 감탄사 + 반응어로만 구성된 패턴만 필터
...
async function generateSingleComment(
    ...
    // 50%: 기존 그대로 (이름 시작 OK)
    // 50%: 캐릭터 이름으로 시작 금지 + 생각하다 던진 느낌
    const diversifyHint = Math.random() < 0.5 ? '' : `
- 캐릭터 이름으로 시작하지 말 것
- 완전한 문장 말고 생각하다 던진 느낌으로. 예:
"이거 일부러 저렇게 그린 거지? 너무 티나는데"
"방금 그거 그냥 넘기면 안 되는 장면 같은데"
"아니 저 표정 왜 저따구로 나옴 뭔가 있는데"
"이거 또 나중에 사실은 하면서 뒤집겠지 뻔하다"
"방금 장면 좀 쎄한데 이거 그냥 넘어가면 이상한데"
"아니 저거 아무 의미 없는 장면이면 더 이상한데"
주어 생략 결론 없음 문장 덜 끝나도 됨`;
...
- ~다 어미 금지 (미쳤음/ㅁㅊ OK)
- 이모지 금지
- 작품 전반 평가 금지 ("작가님 천재" 류 금지)${diversifyHint}
${genreHint}
...
// comment style class — 댓글 의도 다변화
// DC 계열 50% (날 것+추리형) / 기존 스타일 50%
// DC 내부: 구체적(60%) + 흐릿한(40%) 자연스럽게 섞임
const COMMENT_STYLES: { weight: number; hint: string }[] = [
    // ── 기존 스타일 (합산 50) ─────────────────────────────────────
    { weight: 20, hint: '방금 읽은 장면에 즉각적으로 감정 반응해.' },
    { weight: 15, hint: '이 장면에서 생긴 궁금증이나 의문을 짧게 표현해.' },
    { weight: 10, hint: '이 전개가 다음에 어떻게 될지 짧게 예측해.' },
    { weight: 5,  hint: '캐릭터 행동이나 설정의 의미를 짧게 해석해.' },
    // ── DC 날 것 — 구체적 (15) ────────────────────────────────────
    { weight: 15, hint: `짧고 날 것의 DC식 반응. 이 화에 나온 인물명·행동 1개 반드시 언급.
예: "아 거기서 칼 꺼내네 진짜" "로젠 저 장면 왜 저랬냐" "이번화 [인물] 폼 미쳤음ㅋ" "이거 [사건] 설정 구멍 아님?"` },
    // ── DC 날 것 — 흐릿한 (10) ────────────────────────────────────
    { weight: 10, hint: `짧고 날 것의 DC식 반응. 구체 언급 없이 즉흥. 완전한 문장 말고 생각하다 던진 느낌으로.
...
"아니 저거 아무 의미 없는 장면이면 더 이상한데"` },
    // ── DC 추리형 — 구체적 (17) ───────────────────────────────────
    { weight: 17, hint: `독자가 작가 의도/떡밥을 탐지하는 추리형. 이 화 장면·인물 1개 반드시 언급.
예: "작가 또 [장면] 초반에 힘 빼놨네 나중에 터뜨리려고ㅋㅋ"
"방금 [장면] 다시 보니까 앞에 복선이랑 연결되는 거 같은데?"
"지금 [인물] 평범해 보이는데 설정 공개되면 평가 뒤집힐 듯"` },
    // ── DC 추리형 — 흐릿한 (8) ────────────────────────────────────
    { weight: 8, hint: `독자가 작가 의도/떡밥을 탐지하는 추리형. 구체 언급 없이. 완전한 문장 말고 생각하다 던진 느낌으로.
예: "이거 또 나중에 '사실은~' 하면서 뒤집겠지 뻔하다"
"아니 스토리 흐름 갑자기 왜 저기로 튀냐"
"이거 떡밥 아니면 진짜 작가 감 잃은 거임"
"이거 한 번 꼬아놓을 거 같은데 불안하다" "나만 이상하게 보이냐"` },
];

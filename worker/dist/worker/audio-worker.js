...
const tts_1 = require("../app/lib/tts");
const audio_1 = require("../app/lib/audio");
async function getNextPendingTask() {
    const result = await db_1.default.query(`SELECT novel_id, episode_number, language, voice_name 
     FROM audio_files
...
async function getEpisodeText(novelId, episodeNumber, language) {
    var _a;
    const result = await db_1.default.query(`SELECT et.translated_text
     FROM episode_translations et
     JOIN episodes e ON e.id = et.episode_id
     WHERE e.novel_id = $1 
       AND e.ep = $2 
       AND et.language = $3 
       AND et.status = 'DONE'`, [novelId, episodeNumber, language]);
    return ((_a = result.rows[0]) === null || _a === void 0 ? void 0 : _a.translated_text) || null;
}
...
async function autoQueueCompletedTranslations() {
    try {
        ...
        const result = await db_1.default.query(`SELECT DISTINCT e.novel_id, e.ep, et.language
       FROM episode_translations et
       JOIN episodes e ON e.id = et.episode_id
       ...
       LIMIT 10`);
        if (result.rows.length > 0) {
            ...
            for (const row of result.rows) {
                ...
                const voiceName = (0, tts_1.getVoiceName)(language);
                if (voiceName) {
                    await (0, audio_1.createAudioRecord)(novel_id, ep, language, voiceName);
...
async function processTask(task) {
    ...
    try {
        await (0, audio_1.updateAudioStatus)(novel_id, episode_number, language, "PROCESSING");
        const text = await getEpisodeText(novel_id, episode_number, language);
        if (!text) {
            throw new Error(`No text content found for ${language}`);
        }
        const processedText = (0, tts_1.preprocessText)(text);
        const chunks = (0, tts_1.splitIntoChunks)(processedText, 4500);
        console.log(`[AudioWorker] Processing ${chunks.length} chunks`);
        let audioData;
        if (chunks.length === 1) {
            audioData = await (0, tts_1.generateAudio)(chunks[0], language);
        }
        else {
            const audioChunks = await Promise.all(chunks.map((chunk) => (0, tts_1.generateAudio)(chunk, language)));
            audioData = Buffer.concat(audioChunks);
        }
        const duration = (0, audio_1.calculateDuration)(audioData);
        const audioUrl = await (0, audio_1.saveAudioFile)(novel_id, episode_number, language, audioData);
        const fileSize = await (0, audio_1.getAudioFileSize)(novel_id, episode_number, language);
        await (0, audio_1.updateAudioStatus)(novel_id, episode_number, language, "DONE", {
            audioUrl,
        ...
        await (0, audio_1.updateAudioStatus)(novel_id, episode_number, language, "FAILED", {
